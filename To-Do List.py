...
class App(QMainWindow):
	...
	def setMenuBarAction(self): # add Action to the menu bar...
		...
		self.Documention = QAction('Documention', self) # Documention Action.. 
	...
	def appMenuBar(self):
		...
		self.helpMenu.addAction(self.Documention) # Documention Action Added to Help Menu..
	...
	@pyqtSlot()
	def fun(self):
		choice = QMessageBox.question(self, 'Task Reminder',
	...
	def onClick(self):
		...
		text = f'Date: {date.toPyDate()} Time: {time.toPyTime()}' # convert to string..
		if textvalue == '': # If textvalue in taskText is blank then return don't update textArea..
			...
			if os.path.exists(self.uncompletedTaskFile):
				...
				with open(self.uncompletedTaskFile, 'w') as f:
					f.write(textvalue+'\n')
					f.write(text+'\n')
			self.textArea.append(textvalue) # else update textArea..
			self.textArea.append(text+'\n')
			self.taskText.setText('')
		...
		if textValue == '':
			...
			if os.path.exists(self.completedTaskFile):
				...
				with open(self.completedTaskFile, 'w') as f:
					f.write(textValue)
			self.completedtextArea.append(textValue) # append to completed task list section..
